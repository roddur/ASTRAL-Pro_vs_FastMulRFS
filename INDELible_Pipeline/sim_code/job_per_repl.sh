...
function job_per_replicate {
    ...
    rm -f $data/$1/$2/trees.txt
    for gtree in ${total_gtrees[@]}; do
        if [ -f $data/$1/$2/g_trees$gtree\.trees ]; then
            cat $data/$1/$2/g_trees$gtree\.trees >> $data/$1/$2/trees.txt
        fi
    done

    for seqlen in ${sqlens[@]}; do
        rm -rf $data/$1/$2/$seqlen
        mkdir $data/$1/$2/$seqlen
        ...
        python $setindeliblepy \
            ...
            -l $seqlen \
            -o $data/$1/$2/indelible-parameters.csv
        ...
        python $indeliblepy \
            ...
            -s 1 \
            -e $n_total_gtrees \
            -p $data/$1/$2/indelible-parameters.csv \
            -t $data/$1/$2/trees.txt \
            -o $data/$1/$2/$seqlen

	    rm -f $data/$1/$2/trees_est\_$seqlen.txt

        # runs FastTree on phylip files. I've also copied the trees in trees_est_$seqlen.txt files. 
        for gtree in ${total_gtrees[@]}; do
            $fasttree -nt -gtr $data/$1/$2/$seqlen/$gtree\.phy > $data/$1/$2/$seqlen/$gtree\.nwk
            $data/$1/$2/$seqlen/$gtree\.nwk >> $data/$1/$2/trees_est\_$seqlen.txt
        done
        rm -f $data/$1/$2/indelible-parameters.csv
    done

}

module load python/2
job_per_replicate $1 $2
